...
export class CidadeDAO {
    public static buscaTodasCidadesPorEstado(estado_id: number): Promise<Cidade[]> {
            ...
            clientFactory
                ...
                .then((result: QueryResult) => {
                    ...
                    if (result.rows.length > 0) {
                        retorno = [];
                        result.rows.map(dado => retorno.push(new Cidade(dado.id, dado.nome, estado_id)));
                    }
                    resolve(retorno);
                })
                .catch(error => {
                    logger.error(`cidade.dao.buscaTodasCidadesPorEstado - ${error}`);
                    reject(new Mensagem(`Erro ao tentar recuperar as cidades do estado ${estado_id}.`, "erro"));
                });
        });
    }
        ...
        return new Promise((resolve, reject) => {
            clientFactory
                ...
                .catch(error => {
                    ...
                    reject(new Mensagem(`Erro ao tentar recuperar a cidade ${id}.`, "erro"));
                });
        });
    }
}